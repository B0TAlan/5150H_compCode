 ...
 task spinPring();
  ...
  void liftUp(){
    ...
    if(Lift1.position(rev) < 1.35){
       ...
       waitUntil(Lift1.isDone());
       liftH1 = true;

    }
    else if (Controller1.ButtonR1.pressing() && liftH1 == true){
       Lift1.setVelocity(200, percent);
        //Lift1.spinFor(forward, 2, rev);
        Lift1.rotateTo(2, rev);
        waitUntil(Lift1.isDone());
        liftH1 = false;
    }
    else {
  ...
  void bLDown(){//down 
    ...
    //Lift1.spinFor(reverse, 2, rev);
    bL.rotateFor(-1.7,rev, false);
  }
  void bLUp(){//up
    bL.setVelocity(200, percent);
    bL.rotateTo(-3.3,rev, false);
  ...
  void set4Pring(){
    if (Lift1.position(rotationUnits::rev) <= .2){
      ...
      waitUntil(Lift1.position(rev) == .2);
      Lift1.stop();    
    }
    else {
    }
