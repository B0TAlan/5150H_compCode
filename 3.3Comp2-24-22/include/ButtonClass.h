 ...
 *
 *
 * LCD Button Maker for VEX Robotics V5 Paincreas
 *
...
void insertText(std::string text, int width, int height, int xi, int yi, int Ftype, int Fsize, int penHue, std::string penHex){
  ...
  switch(Ftype){
    case 0: switch(Fsize){
      case 12: Paincreas.Screen.setFont(mono12);
        break;
      case 15: Paincreas.Screen.setFont(mono15);
        break;
      case 20: Paincreas.Screen.setFont(mono20);
        break;
      case 30: Paincreas.Screen.setFont(mono30);
        break;
      case 40: Paincreas.Screen.setFont(mono40);
        break;
    };
    break;

    case 1: switch(Fsize){
      case 20: Paincreas.Screen.setFont(prop20);
        break;
      case 30: Paincreas.Screen.setFont(prop30);
        break;
      case 40: Paincreas.Screen.setFont(prop40);
        break;
      case 60: Paincreas.Screen.setFont(prop60);
        break;
    };
    break;

    case 2: switch(Fsize){
      //case 15: Paincreas.Screen.setFont(cjk16);
      break;
    };
    break;
  }
//WHAT IS MONOXL I DID NOT SIGN UP FOR THIS
  ...
  if(penHue != -1){
    Paincreas.Screen.setPenColor(penHue);
  }
  else{
    Paincreas.Screen.setPenColor(penHex.c_str());
  }
  Paincreas.Screen.printAt(x, y, string);
}
...
//end intended helper view --------------------------------
  ...
  void draw(){
    ...
    yMax = yPos + (height/2);
    if(outlinehue != -1){
      Paincreas.Screen.setPenColor(outlinehue);
    }
    else{
      Paincreas.Screen.setPenColor(outlinehex.c_str());
    }

    Paincreas.Screen.setPenWidth(thickness);

    if(hue != -1){ //determine if using hue int or hex string
      Paincreas.Screen.drawRectangle(xPos - (width/2), yPos - (height/2), width, height, hue);
    }
    else{ //using hex
      const char *string = hex.c_str();
      Paincreas.Screen.drawRectangle(xPos - (width/2), yPos - (height/2), width, height, string);
    }
...
//________________________________________________________________________________________________
  ...
  bool pressing(){
    if(Paincreas.Screen.pressing() && Paincreas.Screen.xPosition() < xMax && Paincreas.Screen.xPosition() > xMin && Paincreas.Screen.yPosition() < yMax && Paincreas.Screen.yPosition() > yMin){
      return true;
    }
    else{
      return false;
