...
void Skills(){
  ...
  aC.Open(); // open clamp
  driveBase.driveFor(fwd, 1.5, distanceUnits::in, 20, velocityUnits::pct);// drives twords blue mgoal
  aC.Cloes();// clamps the blue mgoal
  aL.ClearWall(100, true);// raises the lift and blue mgoal over the wall allowing it to turn 
  driveBase.turnToHeading(90, rotationUnits::deg);// turn to aline with yello mgoal
  ...
  driveBase.driveFor(fwd, 40, distanceUnits::in, 40, velocityUnits::pct);
...
void NoTon(){
  //Pid.Turn(90, .45);
  //driveBase.driveFor(fwd, 10, distanceUnits::in);
  //driveBase.turnToHeading(90, degrees);
  Pid.Drive(10, .01,.0000001, 0.1);
}