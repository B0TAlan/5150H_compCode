  ...
  DriveBaseControler DriveMotors;
   DriveMotors.Reset();// reset all motors encoders to 0
    ...
    while(stopPid == false){
      ...
      if ((fabs(rightError)<= 100) && (fabs(leftError)<= 100)){//
...
void Movement_Controller::customTurn(float heading, float kp, float kd, float ki, bool left, bool right){
  DriveBaseControler DriveMotors;
...
void Movement_Controller::Point(float heading, float kp){
  customTurn(heading, kp, .00001, 0.1, true, true);
}

void Movement_Controller::Pivot(float heading, float kp, bool Dir/*true == left, flase == right*/){

  if (Dir == true) {
    customTurn(heading, kp, .00001, 0.1, true, false);
  }else if (Dir == false) {
    customTurn(heading, kp, .00001, 0.1, false, true);
  }
}
...
void Movement_Controller::turnFor(float amount, float kp, bool Point, bool Dir){
  ...
  if (Point == true){
    customTurn(target, kp, .00001, 0.1, true, true);
  }else if (Point == false && Dir == true) {
    customTurn(target, kp, .00001, 0.1, true, false);
  }else if (Point == false && Dir == false) {
    customTurn(target, kp, .00001, 0.1, false, true);
  }
}