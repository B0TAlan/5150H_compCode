...
void LiftShortcut::Max(int vel, bool wait) { // raises the lift to the max height
  unLock();
  Lift.rotateTo(2.3, rotationUnits::rev, vel, velocityUnits::pct, wait);
  if(Lift.position(rev) >= 2.4){
    lock();
  }
}
...
void LiftShortcut::Min(int vel, bool wait) { // returns to 0 the lowest point
  unLock();
  Lift.rotateTo(.09, rotationUnits::rev, vel, velocityUnits::pct, wait);
