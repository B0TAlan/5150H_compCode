  ...
  DriveBaseControler DriveMotors;
   DriveMotors.Reset();// reset all motors encoders to 0
    ...
    while(stopPid == false){
     ...
     if(fabs(avgPut) > fabs(powCap)){
       leftPut = powCap;
       rightPut = powCap;
     }

     if(fabs(avgPut) > maxPow){
       leftPut = maxPow * sign;
       rightPut = maxPow * sign;
     }
...
void Movement_Controller::Drive(float target, float Volts){
    ...
    customDrive(target, .15, .1, .15 /*.05, .01 == moon walk*/, Volts);
...
void Movement_Controller::Point(float heading, float Volts){
  customPoint(heading, .13, 9.8, .00005, Volts);
}

void Movement_Controller::Pivot(float heading, float Volts){
customPivot(heading, .1, 0.003, 0.000045, Volts);
}
...
void Movement_Controller::turnFor(float amount, float Volts, bool Point){
  ...
  if (Point == true){
    ...
    customPivot(target, .1, 0.004, 0.00004, Volts);
  }else{}
}