...
void Movement_Controller::customDrive(float target, float kp, float kd, float ki){
  DriveBaseControler DriveMotors;
   DriveMotors.Reset();// reset all motors encoders to 0
    ...
    float acceleration = 0, sign = 0, volCap = 0, Volts = 127;
...
void Movement_Controller::customPivot(float heading, float kp, float kd, float ki, /*bool right, bool left,*/ vex::turnType Dir){//(bool l, bool r)
  DriveBaseControler DriveMotors;
...
void Movement_Controller::Pivot(float heading, float kp, vex::turnType Dir/*true == left, flase == right*/){

  if (Dir == left) {
    customPivot(heading, kp, 9.8, .00005, left);
  }else if (Dir == right) {
    customPivot(heading, kp, 9.8, .00005, right);
  }
}

void Movement_Controller::turnFor(float amount, float kp, bool Point, vex::turnType Dir){
  ...
  if (Point == true){
    ...
    customPivot(target, kp, 9.8, .00005, left);
  }else if (Point == false && Dir == right) {
    customPivot(target, kp, .1, .000055, right);
  }
}