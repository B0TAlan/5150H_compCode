...
void pringleIntake() {
  if (Controller2.ButtonR2.pressing()) {
    pringle.spin(fwd, 255, rpm);

  } else if (Controller2.ButtonR1.pressing()) {
    pringle.spin(reverse, 255, rpm);
...
void clampOpen() { Clamp.set(true); }

void clampCloes() { Clamp.set(false); }

bool bLD = false;

void bLOpen() { bL.set(false); }

void bLClose() { bL.set(true); }
