...
void Skills(){
  ...
  aC.Open(); // open clamp
  driveBase.driveFor(fwd, 8, distanceUnits::in, 20, velocityUnits::pct);// drives twords blue mgoal
  aC.Cloes();// clamps the blue mgoal
  aL.ClearWall(100, true);// raises the lift and blue mgoal over the wall allowing it to turn 
  driveBase.turnToHeading(93, rotationUnits::deg);// turn to aline with yello mgoal
  bL.set(true);// makes sure the back lift is open
  driveBase.driveFor(fwd, -20, inches, 15, velocityUnits::pct);// drives tworads yello mgoal to grab it with bL
  aL.Idle(100, true);// lowers the lift to avoid tipping  
  driveBase.turnToHeading(90, rotationUnits::deg);// corrects its self
  driveBase.driveFor(fwd, -30, inches, 30, velocityUnits::pct);// drives twords the yello mgoal
  driveBase.driveFor(fwd, -14, inches, 10, velocityUnits::pct);// slows down to have yello magoal slide into back lift 
  ...
  driveBase.driveFor(fwd, -20, distanceUnits::in, 30, velocityUnits::pct);// backs away from the platform
  ...
  driveBase.turnToHeading(191, rotationUnits::deg);// rotates to face red mgoal
  aB.Open();// opens bL to drop yello mgoal
  aC.Open();// makes sure clamp is open
  //-----------------------------2nd part--------------------------------------//
  driveBase.driveFor(fwd, 36, distanceUnits::in, 30, velocityUnits::pct);// drives twords  red mgoal
  driveBase.driveFor(fwd, 7, distanceUnits::in, 10, velocityUnits::pct, false);// slows down in order to not crash
  aC.Cloes();// grabes mgoal
  driveBase.driveFor(fwd, -15, distanceUnits::in, 60, velocityUnits::pct);//dive back to aline with middle mgoal and to avoids pringles
  aL.Idle(100, true);// raises lift to idle height for smoother turning
  driveBase.turnToHeading(221, rotationUnits::deg);// turn twords middle mgoal
  driveBase.driveFor(fwd, -34, distanceUnits::in, 30, velocityUnits::pct);// drives twords the middle mgoal  
  //aL.Min(100, false);
  driveBase.driveFor(fwd, -15, distanceUnits::in, 15, velocityUnits::pct);// nessles middle mgoal into bL
  aB.Close();// close back lift 
  driveBase.turnToHeading(221, rotationUnits::deg);// self correct 
  driveBase.driveFor(fwd, -80, distanceUnits::in, 15, velocityUnits::pct);// drive back to put middle mgoal into the conner next to the blue mgoal
  aB.Open();// releases middle mgoal in the conner
  driveBase.driveFor(fwd, 6, distanceUnits::in, 15, velocityUnits::pct);// moves out of the conner in order to get next blue mgoal
  driveBase.turnToHeading(128, rotationUnits::deg);// turns twords blue mgoal so its alines with bL 
  //aL.ClearWall(100, true);
  driveBase.driveFor(fwd, -16, distanceUnits::in, 15, velocityUnits::pct);// nessles blue mgoal into bL
  aB.Close();// close bL 
  driveBase.turnToHeading(182, rotationUnits::deg);// turns to be parralle to the platform 
  ...
  driveBase.turnToHeading(135, rotationUnits::deg);// turns to put red mgoal on the plat form
  driveBase.driveFor(fwd, 4, distanceUnits::in, 15, velocityUnits::pct);// moves forward to make sure rd mgoal is ove the platform 
  aC.Open();// drops red mgoal balancing it
  driveBase.driveFor(reverse, 4, distanceUnits::in, 15, velocityUnits::pct);// backs away form the platform 
  aL.Min(100, true);// lowers the lift to 0
  driveBase.turnToHeading(180, rotationUnits::deg);// turns back parralle to the platform 
  driveBase.driveFor(reverse, 34, distanceUnits::in, 30, velocityUnits::pct);// drive back to aline with yello mgoal
  driveBase.turnToHeading(270, rotationUnits::deg);// turns to face yello mgoal
  driveBase.driveFor(fwd, 30, distanceUnits::in, 80, velocityUnits::pct);// drive twords yello mgoal 
  driveBase.driveFor(fwd, 5, distanceUnits::in, 40, velocityUnits::pct);// nessles yello mgoal in clamp
